...
class VirtualTerminalPage:
    ...
    add_tip_custom_amount_xpath = "//input[@placeholder='Custom']"
    virtual_pin_input_xpath = "//input[@placeholder='Enter 16 Digit PIN']"
    vt_submit_button_xpath = "//button[normalize-space()='Submit']"
    ...
    def scroll_and_click_VT_submit_button(self):
        scroll_and_click_button(self.driver,By.XPATH,self.vt_submit_button_xpath,timeout=20)
    ...
    def enter_amount_into_custom_tip_field(self,text):
        cu_ip_field = wait_for_element(self.driver,By.XPATH,self.add_tip_custom_amount_xpath,timeout=20,condition="visible")
        cu_ip_field.send_keys(text)
